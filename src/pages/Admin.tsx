...
type Tab = 'gallery' | 'rosters' | 'comments' | 'recruitment' | 'esportsPages' | 'messageBoard' | 'groupChat' | 'topTeams' | 'settings';
...
export default function Admin() {
  ...
  return (
    ...
    >
      ...
      <div className={`flex p-1.5 rounded-2xl w-fit mx-auto sm:mx-0 overflow-x-auto max-w-full ${darkMode ? 'bg-zinc-900' : 'bg-zinc-100'}`}>
        {(['gallery', 'rosters', 'comments', 'recruitment', 'esportsPages', 'messageBoard', 'groupChat', 'topTeams', 'settings'] as Tab[]).map((tab) => (
          <button
            key={tab}
          ...
          >
            {tab === 'gallery' ? 'Photos' : tab === 'rosters' ? 'Rosters' : tab === 'comments' ? 'Comments' : tab === 'recruitment' ? 'Recruitment' : tab === 'esportsPages' ? 'Pages' : tab === 'messageBoard' ? 'Board' : tab === 'groupChat' ? 'Group' : tab === 'topTeams' ? 'Top Teams' : 'Settings'}
            {tab !== 'settings' && (
      ...
      {activeTab === 'settings' ? (
        ...
        <>
          {activeTab !== 'settings' && ['gallery', 'rosters', 'recruitment', 'esportsPages', 'comments'].includes(activeTab) && (
          ...
          {activeTab !== 'settings' && !['gallery', 'rosters', 'recruitment', 'esportsPages'].includes(activeTab) && (
            <div className="mb-8">
              <h2 className={`text-xl font-black uppercase italic ${darkMode ? 'text-white' : 'text-zinc-900'}`}>
            ...
            {items.map((item) => (
              ...
              >
                ...
                <div className={`grid grid-cols-2 gap-3 mt-6 pt-6 border-t ${darkMode ? 'border-zinc-800' : 'border-zinc-50'}`}>
                  {!showApproved && ['gallery', 'rosters', 'recruitment', 'esportsPages', 'comments'].includes(activeTab) && (
                    <button
      ...
      {activeTab !== 'settings' && items.length === 0 && (
        <div className={`text-center py-20 rounded-[2rem] border-2 border-dashed ${darkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-100'}`}>
          <p className="text-zinc-300 font-black uppercase italic text-2xl">
            {['gallery', 'rosters', 'recruitment', 'esportsPages', 'comments'].includes(activeTab) && !showApproved ? 'No Pending Items' : 'No Items Found'}
          </p>
